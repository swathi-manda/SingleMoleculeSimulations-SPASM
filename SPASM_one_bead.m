classdef SPASM_one_bead < handle
% SPASM_one_bead Builds an app with the ability to detect binding events from noisy single
% molecule data and produce relevant figures including ensemble averages.
%   SPASM_one_bead creates a new app instance and displays a GUI for interacting with the
%   app.
%
%   SPASM_one_bead(txtInputFcn) accepts either a function handle or a character vector
...
%
%   SPASM_one_bead(txtInputFcn, txtOutputFcn) also accepts either a function handle or a
...
%
%   myApp = SPASM_one_bead( ___ ) returns the app instance in myApp. Use dot indexing to
...
%
% This program uses the Signal Processing Toolbox to smooth the variance with a
...
% This program should be compatible with MATLAB releases R2017b through at least R2020a.
    ...
    properties (SetObservable, AbortSet)
        ...
        autop1               % The lower peak of the variance histogram, which is used in the peak-to-peak method.
        autop2               % The upper peak of the variance histogram, which is used in the peak-to-peak method.
        automin              % The minimum value between the two peaks of the variance histogram, which is used in the threshold method.
    ...
    properties
        ...
        data      % Contains relevant data for tab 1 (bead position, sampling frequency, ensemble averages, e.g.).
    ...
    methods
        ...
        function app = SPASM_one_bead(txtInputFcn, txtOutputFcn)
            % Constructor.
            ...
            try
                ...
                if nargin >= 1 && ~isdeployed
                    ...
                    if isa(txtInputFcn, 'function_handle')
                        ...
                        try
                            ...
                            % An error could arise for any number of reasons, including if
                            % the file is not found. Use the default function and warn the
                            % user.
                    ...
                    try
                        ...
                        if strcmp(ME.identifier, 'MATLAB:UndefinedFunction')
                            % ...and, if the error indicates that the function is not
                            % defined, just use the default function and warn the user.
                ...
                if nargin >= 2 && ~isdeployed
                    ...
                    if isa(txtOutputFcn, 'function_handle')
                        ...
                        % If they did not supply a function handle, assume it is a
                        % character vector containing the name of a .m file:
                        try
        ...
        function trimBead = getTrimBead(app)
            % GETTRIMBEAD Returns the current bead.
            %   trimBeads = GETTRIMBEAD(app) returns app's current bead, excluding data
            %   which was removed by the user.
            
            trimBead = app.data.origBead(app.controls.deselect.indices, :);
        end
        ...
        function events = getActiveEvents(app)
            ...
            %   after each event within the app's bead), the second column gives the event
            %   ends (the last index of each event), and the rows are ordered
            %   chronologically.
        ...
        function sep = getActiveSep(app)
            ...
            events = app.getActiveEvents;
            if ~isempty(events)
                sep = [events(:,1); length(app.getTrimBead)] - [1; events(:,2)];
            else
                sep = [];
            end
        end
        
        function [pos, steps] = getPosNStepSizes(app)
            % GETPOSNSTEPSIZES Returns the average position of the bead just before and
            % just after binding and unbinding, as well as the estimated step sizes.
            %   [pos, steps] = GETPOSNSTEPSIZES(app) returns the average position, in nm,
            %   of the bead just before and just after each currently active event's onset
            %   and conclusion, as well as the estimated step sizes based on these
            %   positions.
            %
            ...
            %   events ordered chronologically:
            %     The first column contains estimates for the size of the bead's
        ...
        function force = getForce(app)
            % GETFORCE Returns the force on the bead before, during, and after each event.
            %   force = GETFORCE(app) returns the force, in pN, of the bead before,
            %   during, and after each currently active event.
            %
    ...
    methods (Static)
        ...
        function [fs, K, CAL, tData, header] = defaultTxtInput(file)
            % DEFAULTTXTINPUT Reads .txt file and returns the contents.
            %   [fs, K, CAL, tData, header] = DEFAULTTXTINPUT(file) assumes file is a text
            ...
            %
            %   (2) There must be keys labeled 'K1' (or 'K'), 'CAL1' (or 'CAL'), and
            %   'Sample Rate', with corresponding numeric values.
            %
            %   (3) One of the column names must equal 'Trap1X' (or 'TrapX'). The
            %   corresponding column will be treated as the data for the bead.
            %
            %   The outputs fs, K, and CAL are the sampling rate, stiffness for the bead,
            %   and volt-to-nanometer conversion factor for the bead, respectively.
            %
            %   The output tData is a table containing the data, with column names
            %   determined by MATLAB from colName1 through colNameM and with the column
            %   for the bead renamed from 'Trap1X' (or 'TrapX') to 'BeadPos'. The data in
            %   these columns are in units of volts.
            %
            ...
            % Read fs, K, and CAL from the header.
            K = str2double(header{2}{strcmp(header{1}, 'K1') | strcmp(header{1}, 'K')});
            CAL = str2double(header{2}{strcmp(header{1}, 'CAL1') | strcmp(header{1}, 'CAL')});
            fs = str2double(header{2}{strcmp(header{1}, 'Sample Rate')});
            
            % Read the data into a table.
            tData = readtable(file, 'HeaderLines', i-1);
            
            % Rename the column that contains the data.
            tData.Properties.VariableNames{strcmp(tData.Properties.VariableNames, 'Trap1X') |...
                strcmp(tData.Properties.VariableNames, 'TrapX')} = 'BeadPos';
        end
        ...
        function var = calculateVar(bead, averagingWindow, smoothingWindow, tlbxAvailable)
            % CALCULATEVAR Calculates the variance of a series of data.
            %   var = CALCULATEVAR(bead, averagingWindow) calculates the sliding window
            %   variance of bead with window size determined by averagingWindow.
            %
            %   var = CALCULATEVAR(bead, averagingWindow, smoothingWindow) passes the
            %   variance through a second order Savitzky-Golay filter with window size
            ...
            %
            %   var = CALCULATEVAR(bead, averagingWindow, smoothingWindow, tlbxAvailable)
            ...
            if nargin < 5
                tlbxAvailable = SPASM_one_bead.checkInstall('Signal Processing Toolbox');
            end
            
            % Calculate the variance.
            bead_filt = movmean(bead, averagingWindow);
            beadbead_filt = movmean(bead.*bead, averagingWindow);
            var = beadbead_filt - bead_filt.*bead_filt;
            % REQUIRES SIGNAL PROCESSING TOOLBOX
            if tlbxAvailable && numel(var) > 2 && nargin > 3
                var = sgolayfilt(var, 2, smoothingWindow);
            end
        end
        ...
        function prelimEvents = findPrelimEvents(var, p1, p2)
            % FINDPRELIMEVENTS Finds events using the peak to peak method.
            %   prelimEvents = FINDPRELIMEVENTS(var, p1, p2) uses the variance var and the
            %   peak locations p1 and p2 to find the indices within var which mark the
            ...
            %   chronologically.
            %
            % In the peak to peak method, event starts are found when the variance drops
            % below the upper peak, provided it then drops below the lower peak. Event
            % ends are found when the variance then rises above the upper peak again.
            
            startp2 = [diff(var < p2); 0] == 1; % Find all points where the variance drops below peak 2...
            istartp2 = find(startp2);

            stopp2 = [diff(var < p2); 0] == -1; % ...and all points where the variance rises above peak 2...
            stopp2(1:istartp2(1)) = 0; % ...excluding any such points without a preceding drop as found above...
            istopp2 = find(stopp2);

            startp1 = [diff(var < p1); 0] == 1; % ...and find all points where the variance drops below peak 1.
            istartp1 = find(startp1);

            % Consider events to occur when the variance first drops below peak 2, then
            % drops below peak 1, and finally rises above peak 2.
        ...
        function prelimEvents = findPrelimEventsByMin(var, m)
            % FINDPRELIMEVENTSBYMIN Finds events using the single threshold method.
            %   prelimEvents = FINDPRELIMEVENTSBYMIN(var, m) uses the variance var and the
            %   minimum location m to find the indices within var which mark the
            ...
            %
            % In the single threshold method, event starts are found when the variance
            % drops below the minimum, and event ends are found when the variance rises
            % above the minimum.
            
            index = var < m; % Select areas where the variance is less than the threshold...
        ...
        function [T1, T2, Lr, idx] = changepoint(data, varargin)
            ...
            if numel(data) <= 1500
                ...
                [T1, T2, Lr] = SPASM_one_bead.findTwoChangepoints(data, onlyLr);
                idx = 1:numel(data);
            else
                ...
                subset = data(idx);
                [t1, t2, Lr] = SPASM_one_bead.findTwoChangepoints(subset, onlyLr); % [T1's estimate, T2's estimate, likelihood]
    ...
    methods (Access = private)
        ...
        function initialize(app)
            ...
            % Colors:
            app.misc.colors.bead = [0, 0.447, 0.741];
            ...
            app.misc.colors.var = [0, 0.447, 0.741];
            app.misc.colors.varPatches = [1, 0, 0];
            app.misc.colors.varHist = [0, 0.447, 0.741];
            ...
            app.data.time = [];
            app.load.CAL = [];
            app.load.CALTxt = uicontrol(app.load.panel,...
                'Units', 'normalized',...
                'Style', 'text',...
                ...
                'FontSize', app.misc.fontSize);
            app.load.K = [];
            app.load.KTxt = uicontrol(app.load.panel,...
                'Units', 'normalized',...
                'Style', 'text',...
                'HorizontalAlignment', 'left',...
                'Visible', 'off',...
            ...
            app.load.main.ylim = [];
            app.load.main.beadPlot = plot(app.load.main.mainAxes, 0, 0,...
                'Color', app.misc.colors.bead,...
                'Visible', 'off',...
                'HitTest', 'off');
            ...
            % Variance axes of load panel of tab 1:
            app.load.var.varAxes = axes(app.load.panel,...
                'Box', 'on',...
                'Visible', 'off',...
                ...
                'YAxisLocation', 'right',...
                'UserData', 'varAxes');
            xlabel(app.load.var.varAxes, 'time (s)');
            ylabel(app.load.var.varAxes, 'variance');            
            app.load.var.varPlot = plot(app.load.var.varAxes, 0, 0,...
                'Color', app.misc.colors.var,...
                'Visible', 'off',...
                'HitTest', 'off');
            app.load.var.peak1Plot = plot(app.load.var.varAxes, 0, 0,...
                'Color', app.misc.colors.peaksMin,...
                'Visible', 'off',...
                'HitTest', 'off');
            app.load.var.peak2Plot = plot(app.load.var.varAxes, 0, 0,...
                'Color', app.misc.colors.peaksMin,...
                'Visible', 'off',...
                'HitTest', 'off');
            app.load.var.minPlot = plot(app.load.var.varAxes, 0, 0,...
                'Color', app.misc.colors.peaksMin,...
                'Visible', 'off',...
                'HitTest', 'off');
            app.load.var.patches = gobjects(0);
            app.load.var.var = [];
            ...
            % Variance histogram axes of analyze panel of tab 1:
            app.analyze.varHist.varHistAxes = axes(app.analyze.panel,...
                'Box', 'on',...
                'Visible', 'off',...
                ...
                'NextPlot', 'add',...
                'YAxisLocation', 'right',...
                'UserData', 'varHistAxes');
            xlabel(app.analyze.varHist.varHistAxes, 'variance');
            ylabel(app.analyze.varHist.varHistAxes, 'count');
            app.analyze.varHist.varHistPlot = histogram(app.analyze.varHist.varHistAxes, [],...
                'FaceColor', app.misc.colors.varHist,...
                'Visible', 'off',...
                'HitTest', 'off');
            app.analyze.varHist.peak1Plot = plot(app.analyze.varHist.varHistAxes, 0, 0,...
                'Color', app.misc.colors.peaksMin,...
                'Visible', 'off',...
                'HitTest', 'off');
            app.analyze.varHist.peak2Plot = plot(app.analyze.varHist.varHistAxes, 0, 0,...
                'Color', app.misc.colors.peaksMin,...
                'Visible', 'off',...
                'HitTest', 'off');
            app.analyze.varHist.minPlot = plot(app.analyze.varHist.varHistAxes, 0, 0,...
                'Color', app.misc.colors.peaksMin,...
                'Visible', 'off',...
                'HitTest', 'off');
            ...
            app.controls.generateVarBtn = uicontrol(app.controls.panel,...
                'Style', 'pushbutton',...
                'String', 'Generate Variance',...
                'Callback', @app.generateVarCallback,...
                'Units', 'normalized',...
            ...
            app.controls.windows.varwindow = 175;
            app.controls.windows.varsmooth = 73;
            app.controls.windows.defvarwindow = app.controls.windows.varwindow;
            app.controls.windows.defvarsmooth = app.controls.windows.varsmooth;
            app.controls.windows.varwindowTxt = uicontrol(app.controls.panel,...
                'Style', 'text',...
                'String', {'Moving average filter window:'; '(for calculating variance)'},...
                'Units', 'normalized',...
                'Position', [0.5, 0.91, 0.38, 0.08],...
                'Enable', 'off',...
                'FontSize', app.misc.fontSize);
            app.controls.windows.varwindowEdit = uicontrol(app.controls.panel,...
                'Style', 'edit',...
                'String', app.controls.windows.defvarwindow,...
                'Units', 'normalized',...
                'Position', [0.88, 0.93, 0.1, 0.05],...
                'Enable', 'off',...
                'Callback', @app.varwindowCallback,...
                'FontSize', app.misc.fontSize);
            app.controls.windows.varsmoothTxt = uicontrol(app.controls.panel,...
                'Style', 'text',...
                'String', {'2nd order Savitzky-Golay filter window:'; '(for smoothing variance)'},...
                'Units', 'normalized',...
                'Position', [0.5, 0.83, 0.38, 0.08],...
                'Enable', 'off',...
                'FontSize', app.misc.fontSize);
            app.controls.windows.varsmoothEdit = uicontrol(app.controls.panel,...
                'Style', 'edit',...
                'String', app.controls.windows.defvarsmooth,...
                'Units', 'normalized',...
                'Position', [0.88, 0.85, 0.1, 0.05],...
                'Enable', 'off',...
                'Callback', @app.varsmoothCallback,...
                'FontSize', app.misc.fontSize);
            ...
            % allEvents becomes outdated when the variance/peaks/min change. Updating
            % allEvents requires running the changepoint algorithm, which is expensive, so
            % the user is always asked before it happens. Therefore, the values of the
            % variance/peaks/min that were used to determine allEvents need to be stored
            % so current values can be compared.
            app.misc.active.allEvents.var = [];
            ...
            [app.excel.cell.path,...
                ...
                app.excel.cell.varwindow,...
                app.excel.cell.varsmooth,...
            ...
            if isprop(app.load.main.mainAxes, 'Toolbar')
                ...
                app.setupAxToolbar(app.load.var.varAxes);
                app.setupAxToolbar(app.analyze.varHist.varHistAxes);
            ...
            if ~app.misc.signal
                warndlg('The Signal Processing Toolbox is needed to smooth the variance.')
            end
        ...
        function sizeChange(app, ~, ~)
            ...
            try
                ...
                if strcmp(app.load.KTxt.Visible, 'on')
        ...
        function defaultClick(app, ~, ~)
            ...
            try
                ...
                switch app.misc.tabgroup.SelectedTab
                    ...
                    case app.misc.tab1
                        axes = [app.load.main.mainAxes,...
                            app.load.var.varAxes,...
                            app.analyze.varHist.varHistAxes,...
                ...
                if ~isempty(a)
                    ...
                    % Just in case there are multiple axes in a, choose the first. (But
                    % there shouldn't be.)
                    a = a(1);
                    ...
                    % Add a submenu for saving the figure as a .fig file, passing the
                    % clicked axes a into the submenu's callback.
                    uimenu(app.misc.cm,...
                        'Text', 'Save as .fig',...
                        'MenuSelectedFcn', {@app.saveFigCallback, a},...
                        'Enable', enableState);

                    % Add a submenu for saving the figure as a .eps file, passing the
                    % clicked axes a into the submenu's callback.
                    uimenu(app.misc.cm,...
                        'Text', 'Save as .eps',...
                    ...
                    if strcmp(a.UserData, 'miscAxes') || strcmp(a.UserData, 'combinedMiscAxes')
                        ...
                        addMenu('Event Duration / Force on Bead');
                    end
        ...
        function exportMiscDistCallback(app, ~, ~, ax)
            ...
            try
                ...
                % Determine which plot is currently showing, and create suggested
                % filename.
                switch ax.UserData
                ...
                % If user picks a file which already exists, first delete the file
                % (because writetable will not overwrite, at least not in R2019a).
                if exist([path name '.xlsx'], 'file') == 2
                  delete([path name '.xlsx']);
                end
                ...
                % Determine labels and parameters to output based on which plot is
                % currently showing.
                switch showing
                    case 'Step 1 Size'
                    ...
                    case 'Total Step Size'
                        ...
                        colNames = {'total step sizes (nm)','probability'...
                            'fit sizes (nm)','fit probability'};
                        rowNames = {'mean (nm)';'standard deviation (nm)'};
                    case 'Event Duration / Force on Bead'
                        T = table; % no parameters to output
                        colNames = {'force on bead (pN)','log(event duration)'};
                    otherwise
                        T = table(kDur);
        ...
        function enableable = getEnableable(app)
            ...
            enableable = [app.load.inputBtn,...
                ...
                app.controls.windows.varwindowEdit,...
                app.controls.windows.varsmoothEdit,...
                ...
                app.excel.findBtn,...
                app.excel.addBtn,...
                app.controls.windows.varwindowTxt,...
                app.controls.windows.varsmoothTxt,...
        ...
        function store = disable(app, varargin)
            ...
            if nargin == 2 && strcmp(varargin{1}, '-toolbars')
                ...
                if app.misc.axToolbars
                    ...
                    app.setAxToolbar(app.load.var.varAxes, 'off');
                    app.setAxToolbar(app.analyze.varHist.varHistAxes, 'off');
        ...
        function restore(app, store)
            ...
            if app.misc.axToolbars
                ...
                app.setAxToolbar(app.load.var.varAxes, 'on');
                app.setAxToolbar(app.analyze.varHist.varHistAxes, 'on');
            ...
            try
                ...
                if strcmp(state, 'on')
                    ...
                    if nargin < 4
                        ...
                        updateSelectionChangedFcn(app.load.var.varAxes, true);
                        updateSelectionChangedFcn(app.analyze.varHist.varHistAxes, true);
                ...
                elseif strcmp(state, 'off')
                    ...
                    updateSelectionChangedFcn(app.load.var.varAxes, false);
                    updateSelectionChangedFcn(app.analyze.varHist.varHistAxes, false);
            ...
            try
                switch src.Name
                    ...
                    case {'autop1', 'autop2', 'automin', 'manp1', 'manp2', 'manmin', 'usemin', 'useman'}
                        % Any of the peaks/min, usemin, or useman have changed, so variance
                        % and variance histogram plots might need to be updated and events
                        % might be outdated.
                        ...
                        % Update variance and variance histogram plots.
                        if strcmp(app.analyze.varHist.varHistAxes.Visible, 'on')
                            app.updateCurrentTask('Plotting the peaks and minimum.');
                        end
                        app.updatePeaksMinPlots(p1, p2, m);
                        drawnow;

                        % As variance/peaks/min may be different, check if events need to be
                        % updated.
...
% Private methods for simulated input
    methods (Access = private)
        ...
        function [pos, steps] = getRealPosNStepSizes(app)
            % Returns the average position of the bead just before and just after binding and
        ...
        function plotRealEvents(app, events)
            % Plots real events on the main axes and variance axes in load panel of tab 1.
            ...
            limyV = ylim(app.load.var.varAxes);
            limyM = ylim(app.load.main.mainAxes);
            liminitC = limyV; liminitM = limyM;
            limyV(1) = limyV(1) - 0.1*(limyV(2) - limyV(1)); % Each patch will span from 10% below...
            limyV(2) = limyV(2) + 0.1*(limyV(2) - limyV(1)); % ...to 10% above current ylim.
            ...
            for nevent = 1:size(events, 1)
                % For each event...
                event = events(nevent,:);
                app.misc.simPatches(end+1) = patch(app.load.var.varAxes,...
                    'XData', [fileTime(event(1)+1) fileTime(event(2)) fileTime(event(2)) fileTime(event(1)+1)],...
                    'YData', [limyV(1) limyV(1) limyV(2) limyV(2)],...
            ...
            ylim(app.load.var.varAxes, liminitC);
    ...
    methods (Access = private)
        ...
        function backup = storeBackupTab1(app)
            ...
            backup.data.origBead = app.data.origBead;
            backup.data.time = app.data.time;
            backup.load.CAL = app.load.CAL;
            backup.load.CALTxt = storeText(app.load.CALTxt);
            backup.load.K = app.load.K;
            backup.load.KTxt = storeText(app.load.KTxt);
            ...
            backup.load.main.beadPlot = storePlot(app.load.main.beadPlot);
            ...
            backup.load.var.varAxes = storeAxes(app.load.var.varAxes);
            backup.load.var.varPlot = storePlot(app.load.var.varPlot);
            backup.load.var.peak1Plot = storePlot(app.load.var.peak1Plot);
            backup.load.var.peak2Plot = storePlot(app.load.var.peak2Plot);
            backup.load.var.minPlot = storePlot(app.load.var.minPlot);
            backup.load.var.patches = storePatch(app.load.var.patches);
            backup.load.var.var = app.load.var.var;
            
            backup.analyze.varHist.varHistAxes = storeAxes(app.analyze.varHist.varHistAxes);
            backup.analyze.varHist.varHistPlot.Data = app.analyze.varHist.varHistPlot.Data;
            backup.analyze.varHist.varHistPlot.Visible = app.analyze.varHist.varHistPlot.Visible;
            backup.analyze.varHist.varHistPlot.NumBins = app.analyze.varHist.varHistPlot.NumBins;
            backup.analyze.varHist.peak1Plot = storePlot(app.analyze.varHist.peak1Plot);
            backup.analyze.varHist.peak2Plot = storePlot(app.analyze.varHist.peak2Plot);
            backup.analyze.varHist.minPlot = storePlot(app.analyze.varHist.minPlot);
            ...
            backup.controls.generateVarBtn = storeButton(app.controls.generateVarBtn);
            ...
            backup.controls.windows.varwindow = app.controls.windows.varwindow;
            backup.controls.windows.varsmooth = app.controls.windows.varsmooth;
            backup.controls.windows.defvarwindow = app.controls.windows.defvarwindow;
            backup.controls.windows.defvarsmooth = app.controls.windows.defvarsmooth;
            backup.controls.windows.varwindowTxt = storeText(app.controls.windows.varwindowTxt);
            backup.controls.windows.varwindowEdit = storeText(app.controls.windows.varwindowEdit);
            backup.controls.windows.varsmoothTxt = storeText(app.controls.windows.varsmoothTxt);
            backup.controls.windows.varsmoothEdit = storeText(app.controls.windows.varsmoothEdit);
            ...
            backup.misc.active.allEvents.var = app.misc.active.allEvents.var;
        ...
        function restoreBackupTab1(app, backup, mode)
            ...
            varQ = any(strcmp(mode, {'all', 'fullReset'}));
            varHistQ = any(strcmp(mode, {'all', 'fullReset'}));
            ...
            if loadQ
                ...
                app.data.origBead = backup.data.origBead;
                app.data.time = backup.data.time;
                app.load.CAL = backup.load.CAL;
                restoreText(app.load.CALTxt, backup.load.CALTxt);
                app.load.K = backup.load.K;
                restoreText(app.load.KTxt, backup.load.KTxt);
            ...
            if mainQ
                ...
                restorePlot(app.load.main.beadPlot, backup.load.main.beadPlot);
                if eventsQ
            ...
            if varQ
                restoreAxes(app.load.var.varAxes, backup.load.var.varAxes);
                restorePlot(app.load.var.varPlot, backup.load.var.varPlot);
                restorePlot(app.load.var.peak1Plot, backup.load.var.peak1Plot);
                restorePlot(app.load.var.peak2Plot, backup.load.var.peak2Plot);
                restorePlot(app.load.var.minPlot, backup.load.var.minPlot);
                if eventsQ
                    app.load.var.patches = restorePatch(app.load.var.patches, backup.load.var.patches,...
                        'FaceColor', app.misc.colors.varPatches,...
                        'FaceAlpha', 0.3);
                end
                app.load.var.var = backup.load.var.var;
            end
            
            if varHistQ
                restoreAxes(app.analyze.varHist.varHistAxes, backup.analyze.varHist.varHistAxes);
                app.analyze.varHist.varHistPlot.Data = backup.analyze.varHist.varHistPlot.Data;
                app.analyze.varHist.varHistPlot.Visible = backup.analyze.varHist.varHistPlot.Visible;
                app.analyze.varHist.varHistPlot.NumBins = backup.analyze.varHist.varHistPlot.NumBins;
                restorePlot(app.analyze.varHist.peak1Plot, backup.analyze.varHist.peak1Plot);
                restorePlot(app.analyze.varHist.peak2Plot, backup.analyze.varHist.peak2Plot);
                restorePlot(app.analyze.varHist.minPlot, backup.analyze.varHist.minPlot);
            end
            ...
            if controlsQ
                restoreButton(app.controls.generateVarBtn, backup.controls.generateVarBtn);
                ...
                app.controls.windows.varwindow = backup.controls.windows.varwindow;
                app.controls.windows.varsmooth = backup.controls.windows.varsmooth;
                app.controls.windows.defvarwindow = backup.controls.windows.defvarwindow;
                app.controls.windows.defvarsmooth = backup.controls.windows.defvarsmooth;
                restoreText(app.controls.windows.varwindowTxt, backup.controls.windows.varwindowTxt);
                restoreText(app.controls.windows.varwindowEdit, backup.controls.windows.varwindowEdit);
                restoreText(app.controls.windows.varsmoothTxt, backup.controls.windows.varsmoothTxt);
                restoreText(app.controls.windows.varsmoothEdit, backup.controls.windows.varsmoothEdit);
            ...
            if eventsQ
                app.misc.active.allEvents.var = backup.misc.active.allEvents.var;
            ...
            if varQ
                restoreAxesLimits(app.load.var.varAxes, backup.load.var.varAxes);
            end
            if varHistQ
                restoreAxesLimits(app.analyze.varHist.varHistAxes, backup.analyze.varHist.varHistAxes);
            end
            if miscQ
            ...
            try
                ...
                try
                    ...
                    try
                        ...
                        [app.data.fs, app.load.K, app.load.CAL,...
                    ...
                    testNumeric = @(a, s) assert(isa(a, 'numeric') && ~isnan(a),...
                        ...
                        s);
                    testNumeric(app.load.K, 'value of K');
                    testNumeric(app.load.CAL, 'value of CAL');
                    ...
                    i = ismember('BeadPos', app.data.origData.Properties.VariableNames);
                    assert(i,...
                        'chooseTxtCallback:outputFailed',...
                        'The table returned by the data input function did not contain a column labeled "BeadPos".');
                    warning('on', 'MATLAB:table:ModifiedAndSavedVarnames')
                catch ME
                    warning('on', 'MATLAB:table:ModifiedAndSavedVarnames')
                ...
                % plot the bead.
                app.data.origBead = app.data.origData.BeadPos*app.load.CAL;
                app.controls.deselect.indices = true(size(app.data.origBead, 1), 1);
                app.data.time = (1:length(app.getTrimBead)) / app.data.fs;
                app.applyHeader;
                app.plotGlobal;
                app.addToNew(true, [app.controls.flipDataBtn,...
                    app.controls.deselect.deselectBtn]);
                app.addToNew(true, app.controls.generateVarBtn);
                app.restore(store);
            catch ME
                if ~isvalid(app)
                    return
        ...
        function applyHeader(app)
            ...
            % Update texts.
            app.load.KTxt.String = ['K_A: ' num2str(app.load.K) ' [pN/nm]'];
            app.load.CALTxt.String = ['CAL_A: ' num2str(app.load.CAL) ' [nm/V]'];
            ...
            drawnow;
            height = 0.04;
            app.load.CALTxt.Position = [app.load.inputBtn.Position(1) + app.load.inputBtn.Position(3) + 0.005,...
                app.load.inputBtn.Position(2) + height, app.load.CALTxt.Extent(3), height];
            app.load.KTxt.Position = [app.load.inputBtn.Position(1) + app.load.inputBtn.Position(3) + 0.005,...
                app.load.inputBtn.Position(2), app.load.KTxt.Extent(3), height];
            app.load.fsTxt.Position = [app.load.CALTxt.Position(1) + app.load.CALTxt.Position(3) + 0.005,...
                app.load.inputBtn.Position(2) + height, app.load.fsTxt.Extent(3), height];
            app.load.filenameTxt.Position = [app.load.KTxt.Position(1) + app.load.KTxt.Position(3) + 0.005,...
            ...
            % Make texts visible.
            app.load.KTxt.Visible = 'on';
            app.load.CALTxt.Visible = 'on';
            app.load.fsTxt.Visible = 'on';
            app.load.filenameTxt.Visible = 'on';
        end
        
        function plotGlobal(app)
            % Updates main axes in load panel of tab 1 with plot of bead over time.
            
            app.updateCurrentTask('Plotting bead position.');
            
            % If bead is already plotted, xlim should be preserved.
            xliminit = [];
            if strcmp(app.load.main.beadPlot.Visible, 'on')
            ...
            % Update plots.
            app.changeData(app.load.main.beadPlot, app.data.time, bead);
            ...
            if strcmp(app.load.var.varAxes.Visible, 'on')
                % If variance axes are visible, XTickLabel and XLabel of main axes
                % should be hidden.
                app.load.main.mainAxes.XTickLabel = [];
                app.load.main.mainAxes.XTick = app.load.var.varAxes.XTick; % Update XTicks to match.
                app.load.main.mainAxes.XLabel.Visible = 'off';
                
                % Resize axes based on yticklabels and ylabel.
                tiC = app.load.var.varAxes.TightInset;
                posC = app.load.var.varAxes.Position;
                tiM = app.load.main.mainAxes.TightInset;
                posM = app.load.main.mainAxes.Position;
                newWidth = min([1 - 2*posC(1) - tiC(3), 1 - 2*posM(1) - tiM(3)]);
                posC(3) = newWidth;
                posM(3) = newWidth;
                app.load.var.varAxes.Position = posC;
            ...
            try
                ...
                try
                    sHeader = app.load.txtOutputFcn(...
                        app.data.fs, app.load.K, app.load.CAL,...
                        app.data.origData, app.load.header);
                catch ME
                    % Display any error to user and exit.
                ...
                app.data.origBead = app.getTrimBead;
                app.controls.deselect.indices = true(size(app.data.origBead,1), 1);
        ...
        function generateVarCallback(app, ~, ~)
            % Called when user clicks generateVarBtn. Calls autoGenerateVar() and enables
            % controls panel of tab 1.
            ...
            try
                ...
                drawnow;
                app.autoGenerateVar;
                app.addToNew(false, app.controls.generateVarBtn);
                app.addToNew(true, [app.controls.windows.varwindowTxt,...
                    app.controls.windows.varwindowEdit,...
                    app.controls.windows.varsmoothTxt,...
                    app.controls.windows.varsmoothEdit,...
                    app.controls.windows.interactBtn,...
        ...
        function autoGenerateVar(app)
            % Calculates the variance, updates the variance and variance histogram plots,
            % updates the automatic peaks/min based on the histogram plot, and updates the
            % preliminary event patches, if needed.
            
            app.updateCurrentTask('Calculating the variance.');
            varwin = app.controls.windows.varwindow;
            varsm = app.controls.windows.varsmooth;
            bead = app.getTrimBead;
            app.load.var.var = app.calculateVar(bead, varwin, varsm, app.misc.signal);
            
            app.updateCurrentTask('Plotting the variance.');
            app.plotVar(app.load.var.var);
            
            app.updateCurrentTask('Plotting the variance histogram.');
            app.plotVarHist(app.load.var.var);
            ...
            if ~isempty(app.load.var.patches)
                % Only update if already showing.
                var = app.load.var.var;
                switch app.useman
                    case false
                        p1 = app.autop1;
                        p2 = app.autop2;
                ...
                switch app.usemin
                    case false
                        prelimEvents = app.findPrelimEvents(var, p1, p2);
                    case true
                        prelimEvents = app.findPrelimEventsByMin(var, m);
                end
        ...
        function varwindowCallback(app, src, ~)
            % Called when user changes the value of varwindowEdit. Makes sure input is
            % valid.
            ...
            try
                ...
                if ~isfinite(d) || ~isreal(d) || d <= 0
                    ...
                    src.String = int2str(app.controls.windows.varwindow);
                    app.restore(store);
                    return
                end
                ...
                % If new value does not match current value, variance will be different.
                change = app.controls.windows.varwindow ~= d;

                % Update edit text and current value.
                src.String = int2str(d);
                app.controls.windows.varwindow = d;

                if change
                    % If variance will be different, recalculate it and update plots.
                    app.autoGenerateVar;
                end
        ...
        function varsmoothCallback(app, src, ~)
            % Called when user changes the value of varsmoothEdit. Makes sure input is
            % valid.
            ...
            try
                ...
                if ~isfinite(d) || ~isreal(d) || d <= 0
                    ...
                    src.String = int2str(app.controls.windows.varsmooth);
                    app.restore(store);
                    return
                end
                ...
                % If new value does not match current value, variance will be different.
                change = app.controls.windows.varsmooth ~= d;

                % Update edit text and current value.
                src.String = int2str(d);
                app.controls.windows.varsmooth = d;
        ...
        function windowsCallback(app, ~, ~)
            ...
            try
                ...
                app.changeData(app.controls.windows.pt, app.controls.windows.varwindow, app.controls.windows.varsmooth); % Place a point at current varwindow and varsmooth.
        ...
        function windowsMotion(app, ~, ~, backup)
            ...
            try
                ...
                if in
                    ...
                    % Otherwise...
                    if app.controls.windows.pt.XData == app.controls.windows.varwindow && app.controls.windows.pt.YData == app.controls.windows.varsmooth
                        % ...if point's position is up to date, reset the click callback and return.
                        return
                    end
                    % ...if point's position is outdated, set it to current varwindow and
                    % varsmooth...
                    app.changeData(app.controls.windows.pt, app.controls.windows.varwindow, app.controls.windows.varsmooth);
                end
                % ...and update variance and variance histogram plots.
                varwindow = round(app.controls.windows.pt.XData);
                varsmooth = 2*round((app.controls.windows.pt.YData+1)/2)-1;
                bead = app.getTrimBead;
                var = app.calculateVar(bead, varwindow, varsmooth, app.misc.signal);
                app.plotVar(var);
                app.plotVarHist(var);
                edges = app.analyze.varHist.varHistPlot.BinEdges;
                values = app.analyze.varHist.varHistPlot.Values;
                ...
                % Update edit texts.
                app.controls.windows.varwindowEdit.String = int2str(varwindow);
                app.controls.windows.varsmoothEdit.String = int2str(varsmooth);
                app.controls.peak1Edit.String = num2str(p1);
                app.controls.peak2Edit.String = num2str(p2);
                app.controls.minEdit.String = num2str(m);
            catch ME
        ...
        function windowsClick(app, ~, ~, store, backup)
            ...
            % is open. When user clicks, regardless of mouse position, varwindow and
            % varsmooth are updated based on point's position and plots are updated
            % accordingly.
            ...
            try
                ...
                drawnow;
                % Update values.
                app.controls.windows.varwindow = round(app.controls.windows.pt.XData);
                app.controls.windows.varsmooth = 2*round((app.controls.windows.pt.YData+1)/2)-1;
        ...
        function defwindowsCallback(app, ~, ~)
            % Called when user clicks windows.resetBtn. Restores varwindow and varsmooth
            ...
            try
                ...
                % If either varwindow or varsmooth is different from its old value, variance
                % will be different.
                change = app.controls.windows.varwindow ~= app.controls.windows.defvarwindow ||...
                    app.controls.windows.varsmooth ~= app.controls.windows.defvarsmooth;

                % Update values.
                app.controls.windows.varwindow = app.controls.windows.defvarwindow;
                app.controls.windows.varsmooth = app.controls.windows.defvarsmooth;
        ...
        function plotVar(app, var)
            % Updates variance axes in load panel of tab 1 with plot of variance over
            % time.
            
            % If this is the first time variance is being plotted...
            init = strcmp(app.load.var.varPlot.Visible, 'off');
            
            % Clear plots of peaks/min and preliminary event patches.
            app.changeData(app.load.var.peak1Plot, [], []);
            app.changeData(app.load.var.peak2Plot, [], []);
            app.changeData(app.load.var.minPlot, [], []);
            for npatch = 1:length(app.load.var.patches)
                app.load.var.patches(npatch).Visible = 'off';
            end
            
            % Plot variance.
            app.changeData(app.load.var.varPlot, app.data.time, var);
            axis(app.load.var.varAxes, 'tight');
            
            if init
                % ...then plot should be made visible and XTickLabel and XLabel of main
                % axes should be hidden.
                app.load.var.varAxes.Visible = 'on';
                app.load.main.mainAxes.XTickLabel = [];
                app.load.main.mainAxes.XTick = app.load.var.varAxes.XTick;
                app.load.main.mainAxes.XLabel.Visible = 'off';
                linkaxes([app.load.main.mainAxes, app.load.var.varAxes], 'x');
            end
        ...
        function plotVarHist(app, var)
            % Updates variance histogram axes in analyze panel of tab 1 with plot of
            % variance histogram.
            
            % If this is the first time variance histogram is being plotted...
            init = strcmp(app.analyze.varHist.varHistPlot.Visible, 'off');
            
            % Clear plots of peaks/min.
            app.changeData(app.analyze.varHist.peak1Plot, [], []);
            app.changeData(app.analyze.varHist.peak2Plot, [], []);
            app.changeData(app.analyze.varHist.minPlot, [], []);
            
            % Plot variance histogram.
            app.analyze.varHist.varHistPlot.Data = var;
            app.analyze.varHist.varHistPlot.NumBins = 100;
            axis(app.analyze.varHist.varHistAxes, 'auto xy');
            
            if init
                % ...then plot should be made visible.
                app.analyze.varHist.varHistPlot.Visible = 'on';
                app.analyze.varHist.varHistAxes.Visible = 'on';
            end
            
            app.analyze.varHist.varHistAxes.YAxis.Exponent = 0;
            
            % Resize axes based on yticklabels and ylabel.
            ti = app.analyze.varHist.varHistAxes.TightInset;
            pos = app.analyze.varHist.varHistAxes.Position;
            pos(3) = 0.33 - pos(1) - ti(3);
            app.analyze.varHist.varHistAxes.Position = pos;
        end
        
        function updatePeaksMinPlots(app, p1, p2, m)
            % Updates variance axes in load panel of tab 1 and variance histogram axes in
            % analyze panel of tab 1 with plots of peak 1 and peak 2 or minimum, depending
            % on usemin.
            ...
            if isempty(p1)
                ...
                if strcmp(app.analyze.varHist.varHistAxes.Visible, 'on')
                    % As long as the variance histogram plot is showing, update the peak
                    % 1 line.
                    limy = ylim(app.analyze.varHist.varHistAxes);
                    app.changeData(app.analyze.varHist.peak1Plot, [p1 p1], limy);
                end
                if strcmp(app.load.var.varAxes.Visible, 'on')
                    % As long as the variance plot is showing, update the peak 1 line.
                    limx = xlim(app.load.var.varAxes);
                    app.changeData(app.load.var.peak1Plot, limx, [p1 p1]);
                end
            end
            ...
            if isempty(p2)
                ...
                if strcmp(app.analyze.varHist.varHistAxes.Visible, 'on')
                    ...
                    % 2 line.
                    limy = ylim(app.analyze.varHist.varHistAxes);
                    app.changeData(app.analyze.varHist.peak2Plot, [p2 p2], limy);
                end
                if strcmp(app.load.var.varAxes.Visible, 'on')
                    % As long as the variance plot is showing, update the peak 2 line.
                    limx = xlim(app.load.var.varAxes);
                    app.changeData(app.load.var.peak2Plot, limx, [p2 p2]);
                end
            end
            ...
            if isempty(m)
                ...
                if strcmp(app.analyze.varHist.varHistAxes.Visible, 'on')
                    % As long as the variance histogram plot is showing, update the
                    % minimum line.
                    limy = ylim(app.analyze.varHist.varHistAxes);
                    app.changeData(app.analyze.varHist.minPlot, [m m], limy);
                end
                if strcmp(app.load.var.varAxes.Visible, 'on')
                    % As long as the variance plot is showing, update the minimum line.
                    limx = xlim(app.load.var.varAxes);
                    app.changeData(app.load.var.minPlot, limx, [m m]);
                end
            end
        end
        ...
        function setManualCallback(app, ~, ~)
            ...
            % the variance histogram axes which will mark peak 1, peak 2, and minimum.
            ...
            try
                ...
                % Clear plots of peaks/min on variance histogram axes and variance axes.
                app.changeData(app.analyze.varHist.peak1Plot, [], []);
                app.changeData(app.analyze.varHist.peak2Plot, [], []);
                app.changeData(app.analyze.varHist.minPlot, [], []);
                app.changeData(app.load.var.peak1Plot, [], []);
                ...
                app.analyze.varHist.varHistAxes.XColor = app.misc.colors.activeAxes;
                app.analyze.varHist.varHistAxes.YColor = app.misc.colors.activeAxes;
                app.analyze.varHist.varHistAxes.ZColor = app.misc.colors.activeAxes;
            catch ME
                if ~isvalid(app)
                    return
                end
        ...
        function manualMotion(app, ~, ~, backup)
            ...
            % clicking the variance histogram plot.
            
            thisMotionCallback = app.misc.fig.WindowButtonMotionFcn;
            try
                [in, p1] = app.inAxes(app.analyze.varHist.varHistAxes); % [whether in axes, mouse position in axes]
                if isequal(app.misc.tab1, app.misc.tabgroup.SelectedTab)
                    if in
                        % Plot a red vertical line at mouse X position.
                        limy = ylim(app.analyze.varHist.varHistAxes);
                        app.changeData(app.analyze.varHist.minPlot, [p1(1) p1(1)], limy);
                        limx = xlim(app.load.var.varAxes);
                        app.changeData(app.load.var.minPlot, limx, [p1(1) p1(1)]);
                    else
                        app.changeData(app.analyze.varHist.minPlot, [], []);
                        app.changeData(app.load.var.minPlot, [], []);
                    end
        ...
        function manualClick(app, ~, ~, store, backup, out)
            ...
            % peaks/min by clicking the variance histogram plot. out contains the X
            ...
            try
                ...
                if in
                    ...
                    switch length(out)
                        case 1
                            % First click.
                            app.changeData(app.analyze.varHist.peak1Plot, [p1(1) p1(1)], limy); % Plot a red vertical line at mouse X position.
                            app.changeData(app.load.var.peak1Plot, limx, [p1(1) p1(1)]);
                            app.misc.fig.WindowButtonDownFcn = {@app.manualClick store backup out}; % Pass out to next click's callback.
                        case 2
                            % Second click.
                            app.changeData(app.analyze.varHist.peak2Plot, [p1(1) p1(1)], limy); % Plot a red vertical line at mouse X position.
                            app.changeData(app.load.var.peak2Plot, limx, [p1(1) p1(1)]);
                            app.misc.fig.WindowButtonDownFcn = {@app.manualClick store backup out}; % Pass out to next click's callback.
                        case 3
                            % Third click.
                            app.misc.fig.WindowButtonMotionFcn = app.misc.storeMotionCallback; % Reset the figure motion callback.
                            ...
                            app.analyze.varHist.varHistAxes.XColor = 'k';
                            app.analyze.varHist.varHistAxes.YColor = 'k';
                            app.analyze.varHist.varHistAxes.ZColor = 'k';
        ...
        function findEventsCallback(app, ~, ~)
            ...
            % variance and peaks/min, plots them, updates them using changepoint
            ...
            try
                ...
                app.updateCurrentTask('Detecting events based on the variance.');
                var = app.load.var.var;
                switch app.useman
                    case false
                        p1 = app.autop1;
                ...
                if ~isempty(prelimEvents)
                    ...
                    % There were no events detected from the variance.
        ...
        function plotPrelimEvents(app, events)
            ...
            for nevent = 1:size(events, 1)
                ...
                event = events(nevent,:);
                app.load.var.patches(end+1) = patch(app.load.var.varAxes,...
                    'XData', [fileTime(event(1)+1) fileTime(event(2)) fileTime(event(2)) fileTime(event(1)+1)],...
                    'YData', [limy(1) limy(1) limy(2) limy(2)],...
                    'FaceColor', app.misc.colors.varPatches,...
                    'FaceAlpha', 0.3,...
            ...
            ylim(app.load.var.varAxes, liminit);
        ...
        function refinedEvents = refineEventsChangepoint(app, events)
            ...
            [numPtsBefore, numPtsAfter] = app.findChangepointWindows(events, size(bead,1));
            ...
            if app.controls.useChangepointBtn.Value
                ...
                for i = 1:N
                    ...
                    window = bead(events(i,1) - numPtsBefore(i):events(i,2) + numPtsAfter(i)); % ...consider a window of data surrounding that event...
            ...
            app.misc.active.allEvents.var = app.load.var.var;
        ...
        function checkOutdatedEvents(app)
            % Compares the current variance, peaks, and min, to those which were used to
            % determine the present events. If events are out of data, enables a button to
            % let the user recalculate them.
            ...
            varEventsChange = ~isequal(app.load.var.var, app.misc.active.allEvents.var);
            ...
            enableFindEvents = varEventsChange || minEventsChange || p1EventsChange || p2EventsChange || useCPChange;
        ...
        function plotMisc(app)
            ...
            switch app.analyze.misc.data
                case 'Step 1 Size'
                    [~, step] = app.getPosNStepSizes;
                    Y = step(:,1);
                    if app.misc.simInput
                        [~, stepr] = app.getRealPosNStepSizes;
                        Yr = stepr(:,1);
                    end
                    xlab = 'step 1 size (nm)';
                    type = 'normcdf';
                ...
                case 'Event Duration / Force on Bead'
                    ...
                    xlab = 'force on bead (pN)';
                    app.updateCurrentTask('Plotting the event durations over the force on the bead.');
                otherwise
                    Y = app.getActiveDur/app.data.fs;
        ...
        function [avgbeg, avgend] = calcEnsemble(app)
            ...
            for i = 1:size(events, 1)
                ...
                if events(i,1) - pts_bef < 1
                    ...
                    event = [bead(1)*ones(pts_bef - events(i,1),1); ...
                        bead(1:events(i,2) - exten_pos)];
                else
                    % ...otherwise determine the event window...
                    event = bead(events(i,1) + 1 - pts_bef:events(i,2) - exten_pos);
                end
                % ...and calculate the extension so that each event beginning is the same
                % size as other event beginnings.
                extension = mean(bead(events(i,2) - exten_pos - app.data.fs/200:events(i,2) - exten_pos))*ones(maxDur - dur(i),1);
            ...
            for i = 1:size(events, 1)
                % For each event...
                if events(i,2) + pts_after > length(bead)
                    % ...if event window ends N points after end of file, then last N
                    % values of event window will match last value in file...
                    event = [bead(events(i,1) + 1 + exten_pos:end); ...
                        bead(end)*ones(pts_after + events(i,2) - length(bead),1)];
                else
                    % ...otherwise determine the event window...
                    event = bead(events(i,1) + 1 + exten_pos:events(i,2) + pts_after);
                end
                % ...and calculate the extension so that each event ending is the same
                % size as other event endings.
                extension = mean(bead(events(i,1) + 1 + exten_pos:events(i,1) + 1 + exten_pos + app.data.fs/200))*ones(maxDur - dur(i),1);
        ...
        function initializeIndividualEvent(app)
            ...
            fileTime = app.data.time;
            app.changeData(app.analyze.individual.eventPlot, fileTime, bead);
            ...
            ylabel(app.analyze.individual.eventAxes, 'position of bead (nm)');
        end
        ...
        function plotIndividualEvent(app)
            ...
            % Get events, current event, bead, and time.
            ...
            bead = app.analyze.individual.eventPlot.YData;
            ...
            eventData = bead(start:stop); % Individual event plot Y data.
            ...
            % Plot the changepoints, means, and variances on top of bead.
        ...
        function addToExcelCallback(app, ~, ~)
            ...
            try
                ...
                colNames = {'index before start (# pts)'...
                    ...
                    'before (s)'...
                    'after (s)'...
                    'pos before attach (nm)'...
                    'pos after attach (nm)'...
                    'pos before detach (nm)'...
                    'pos after detach (nm)'...
                    'step 1 (nm)'...
                    'step 2 (nm)'...
                    'total step (nm)'...
                    'avg force before (pN)'...
                    'avg force during (pN)'...
                    'force before detach (pN)'...
                    'force after detach (pN)'...
                    'avg force after (pN)'};
                ...
                for i = 1:size(paths, 1)
                    ...
                    if fileID ~= -1
                        ...
                        writetable(table({'moving average filter window', app.controls.windows.varwindow}), fullpath, 'Range', [app.getExcelCol(1) app.excel.cell.varwindow], 'Sheet', s, 'WriteVariableNames', 0);
                        writetable(table({'2nd order Savitzky-Golay filter window', app.controls.windows.varsmooth}), fullpath, 'Range', [app.getExcelCol(1) app.excel.cell.varsmooth], 'Sheet', s, 'WriteVariableNames', 0);
        ...
        function toExcel = prepExcel(app)
            ...
            %  Position of the bead before and after binding and unbinding.
            %  Step 1, step 2, and total step.
            %  Force on the bead just before, during, and just after event.
            
            [pos, step] = app.getPosNStepSizes;
            force = app.getForce;
            toExcel = [pos...
                step...
                force];
    ...
    methods (Access = private)
        ...
        function backup = storeBackupTab2(app)
            ...
            backup.data2.bead = app.data2.bead;
            ...
            backup.data2.beadStep1 = app.data2.beadStep1;
            backup.data2.beadStep2 = app.data2.beadStep2;
            backup.data2.beadTotalStep = app.data2.beadTotalStep;
            backup.data2.force = app.data2.force;
        ...
        function restoreBackupTab2(app, backup, mode)
            ...
            app.data2.bead = backup.data2.bead;
            ...
            app.data2.beadStep1 = backup.data2.beadStep1;
            app.data2.beadStep2 = backup.data2.beadStep2;
            app.data2.beadTotalStep = backup.data2.beadTotalStep;
            app.data2.force = backup.data2.force;
        ...
        function chooseXlsxCallback(app, ~, ~)
            ...
            try
                ...
                app.data2.beadStep1 = cell(nSheets, 1);
                app.data2.beadStep2 = cell(nSheets, 1);
                app.data2.beadTotalStep = cell(nSheets, 1);
                app.data2.force = cell(nSheets, 1);
                app.data2.bead = cell(nSheets, 1);
                for i = 1:nSheets
                    ...
                    try
                        ...
                        Step1Idx = contains(colNames, 'step 1');
                        Step2Idx = contains(colNames, 'step 2');
                        TotalStepIdx = contains(colNames, 'total step');
                        ForceDuringIdx = contains(colNames, 'avg force during');
                        ForceAfterIdx = contains(colNames, 'force after detach');
                        
                        testOneMatch(startIdx, 'start (# pts)');
                        testOneMatch(stopIdx, 'end (# pts)');
                        testOneMatch(durIdx, 'duration (# pts)');
                        testOneMatch(Step1Idx, 'step 1');
                        testOneMatch(Step2Idx, 'step 2');
                        testOneMatch(TotalStepIdx, 'total step');
                        testOneMatch(ForceDuringIdx, 'avg force during');
                        testOneMatch(ForceAfterIdx, 'force after detach');
                        
                        testNumeric(xlsxData(:,startIdx | stopIdx), 'events');
                        testNumeric(xlsxData(:,durIdx), 'event durations');
                        testNumeric(xlsxData(:,Step1Idx), 'step 1 sizes');
                        testNumeric(xlsxData(:,Step2Idx), 'step 2 sizes');
                        testNumeric(xlsxData(:,TotalStepIdx), 'total step sizes');
                        testNumeric(xlsxData(:,ForceDuringIdx | ForceAfterIdx), 'forces');
                        ...
                        app.data2.beadStep1{i} = xlsxData(:,Step1Idx);
                        app.data2.beadStep2{i} = xlsxData(:,Step2Idx);
                        app.data2.beadTotalStep{i} = xlsxData(:,TotalStepIdx);
                        app.data2.force{i} = xlsxData(:,ForceDuringIdx)-xlsxData(:,ForceAfterIdx);
                    catch ME
                    ...
                    try
                        ...
                        try
                            % Try using the data input function.
                            [~, ~, CAL, txtData, ~] = app.load.txtInputFcn(txtfile);
                        catch ME
                        ...
                        testNumeric = @(a, s) assert(isa(a, 'numeric') && ~isnan(a),...
                            ...
                            'the %s was not a number.', s);
                        testNumeric(CAL, 'value of CAL');
                        assert(istable(txtData),...
                            'chooseXlsxCallback:outputFailed',...
                            'the final variable returned by the data input function had type %s but should be a table.',...
                            class(txtData));
                        j = ismember('BeadPos', txtData.Properties.VariableNames);
                        assert(j,...
                            'chooseXlsxCallback:outputFailed',...
                            'the table returned by the data input function did not contain a column labeled "BeadPos".');
                    catch ME
                    ...
                    BeadPos = txtData.BeadPos * CAL;

                    try
                        % Determine which data should be removed.
                        deselectIndices = true(length(BeadPos), 1); % Used to determine which indices of bead user opted to remove via deselect.
                        ...
                        while ~isnan(res)
                            ...
                            assert(res >= 1 && res <= length(BeadPos),...
                                'chooseXlsxCallback:badIndices',...
                        ...
                        % Remove that data.
                        app.data2.bead{i} = BeadPos(deselectIndices);
                    catch ME
                ...
                if any(failed)
                    if all(failed)
                        ...
                        app.data2.beadStep1 = app.data2.beadStep1(~failed);
                        app.data2.beadStep2 = app.data2.beadStep2(~failed);
                        app.data2.beadTotalStep = app.data2.beadTotalStep(~failed);
                        app.data2.force = app.data2.force(~failed);
                        app.data2.bead = app.data2.bead(~failed);
        ...
        function steps = getStepSizes2(app)
            % For each event, returns the step 1, step 2, and total step sizes, in a cell
            % array.
            
            steps = [app.data2.beadStep1 app.data2.beadStep2 app.data2.beadTotalStep];
        end
        ...
        function plotMisc2(app)
            ...
            switch app.analyze2.misc.data
                case 'Step 1 Size'
                    step = app.getStepSizes2;
                    Y = cell2mat(step(use,1));
                    xlab = 'step 1 size (nm)';
                    type = 'normcdf';
                    app.updateCurrentTask('Plotting the distribution of substep 1 sizes.');
                case 'Step 2 Size'
                    step = app.getStepSizes2;
                    Y = cell2mat(step(use,2));
                    xlab = 'step 2 size (nm)';
                    type = 'normcdf';
                    app.updateCurrentTask('Plotting the distribution of substep 2 sizes.');
                case 'Total Step Size'
                    step = app.getStepSizes2;
                    Y = cell2mat(step(use,3));
                    xlab = 'total step size (nm)';
                    type = 'normcdf';
                    app.updateCurrentTask('Plotting the distribution of total step sizes.');
                case 'Event Duration / Force on Bead'
                    X = cell2mat(app.data2.force(use));
        ...
        function [avgbeg, avgend] = calcEnsemble2(app)
            ...
            for i = 1:length(bead)
                ...
                % Get the bead, events, and durations for that file.
                fileBead = bead{i};
                fileDurs = durs{i};
                fileEvents = events{i};
                N = size(fileEvents, 1);
                ...
                for j = 1:size(fileEvents, 1)
                    ...
                    if fileEvents(j,1) - pts_bef < 1
                        ...
                        event = [fileBead(1)*ones(pts_bef - fileEvents(j,1),1); ...
                            fileBead(1:fileEvents(j,2) - exten_pos)];
                    else
                        % ...otherwise determine the event window...
                        event = fileBead(fileEvents(j,1) + 1 - pts_bef:fileEvents(j,2) - exten_pos);
                    end
                    % ...and calculate the extension so that each event beginning is the
                    % same size as other event beginnings.
                    extension = mean(fileBead(fileEvents(j,2) - exten_pos - app.data2.fs/200:fileEvents(j,2) - exten_pos))*ones(maxDur - fileDurs(j),1);
                ...
                for j = 1:size(fileEvents, 1)
                    % For each event...
                    if fileEvents(j,2) + pts_after > length(fileBead)
                        % ...if event window ends N points after end of file, then last N
                        % values of event window will match last value in file...
                        event = [fileBead(fileEvents(j,1) + 1 + exten_pos:end); ...
                            fileBead(end)*ones(pts_after + fileEvents(j,2) - length(fileBead),1)];
                    else
                        % ...otherwise determine the event window...
                        event = fileBead(fileEvents(j,1) + 1 + exten_pos:fileEvents(j,2) + pts_after);
                    end
                    % ...and calculate the extension so that each event ending is the same
                    % size as other event endings.
                    extension = mean(fileBead(fileEvents(j,1) + 1 + exten_pos:fileEvents(j,1) + 1 + exten_pos + app.data2.fs/200))*ones(maxDur - fileDurs(j),1);
    ...
    methods (Static, Access = private)
        ...
        function sHeader = defaultTxtOutput(fs, K, CAL, tData, header)
            % DEFAULTTXTOUTPUT Formats the header and column names for a text file.
            %   sHeader = DEFAULTTXTOUTPUT(fs, K, CAL, tData, header) accepts the
            %   variables fs, K, CAL, tData, and header exactly as they were returned by
            %   the app's .txt input function.
            %
            ...
            %   newline characters (\n) separating rows of text. The column for the bead
            %   is renamed from 'BeadPos' to 'Trap1X'.
            ...
            try
                ...
                % txtInputFcn but not a txtOutputFcn. Use the provided fs, K, and CAL to
                % create a header in the default format.
                header = cell(2,1);
                header{1} = {'Sample Rate', 'K1', 'CAL1'};
                header{2} = split(num2str([fs K CAL]));
                prepHeader = [header{1}(:)'; header{2}(:)'];
            end
            
            % Rename the columns that contain the data.
            tData.Properties.VariableNames{'BeadPos'} = 'Trap1X';
